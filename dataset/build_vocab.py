...
# Minimum occurrences for words that will get embeddings
MINCOUNT = 1
...
if __name__ == '__main__':
    ...
    with Path(args.dest, 'vocab.chars.txt').open('w') as f:
    ...
    vocab_tags = set()
    with Path(args.dest, tags('train')).open() as f:
        for line in f:
            vocab_tags.update(line.strip().split())

    with Path(args.dest, 'vocab.tags.txt').open('w') as f:
